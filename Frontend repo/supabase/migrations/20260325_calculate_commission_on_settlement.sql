-- Idempotent upfront commission row for a settled application (matches SubmissionTab / CommissionSettings math).
...
BEGIN
  SELECT a.firm_id,
    ...
    INTO v_firm_id, v_loan, v_client_name
...
REVOKE ALL ON FUNCTION public.calculate_commission_on_settlement(uuid, date) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.calculate_commission_on_settlement(uuid, date) TO authenticated;
GRANT EXECUTE ON FUNCTION public.calculate_commission_on_settlement(uuid, date) TO service_role;
