...
const TaskItem: React.FC<{ task: Task; onToggle: (id: string) => void; onSelect: () => void; }> = ({ task, onToggle, onSelect }) => {
            ...
            <div className="flex items-center justify-between">
                ...
                <div className="flex items-center space-x-3 flex-shrink-0 ml-4">
                    ...
                    {task.assigneeAvatarUrl ? (
                        <img src={task.assigneeAvatarUrl} alt={task.assigneeName} className="h-6 w-6 rounded-full" title={`Assigned to ${task.assigneeName}`} />
                    ) : (
                        <div className="h-6 w-6 rounded-full bg-gray-200 dark:bg-gray-600 flex items-center justify-center" title="Unassigned">
                ...
                <Card className="!p-0 overflow-hidden">
                    ...
                    <ul className="space-y-2 p-4 max-h-[15rem] overflow-y-auto">
                        {overdueTasks.length > 0 ? overdueTasks.map(task => (
                            <TaskItem key={task.id} task={task} onToggle={handleToggleTask} onSelect={() => setSelectedTask(task)} />
